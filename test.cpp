...
//     int fd_c_socket;
// public:
//     Client(int fd) : fd_c_socket(fd) {}
//     int get_fd() const { return fd_c_socket; }
//     void send_message(const char *message) {
//         if (send(fd_c_socket, message, strlen(message), 0) == -1) {
//             throw "send error";
//         }
...
//     void accept_connection();
//     void receive_message();
//     void send_message_all(const char *message);
//     ~Server();
// };
...
//         // Set client socket to non-blocking
//         fcntl(fd_c_socket, F_SETFL, O_NONBLOCK);
//         clients.push_back(new Client(fd_c_socket));
//     }
// }
...
//         } else {
//             buffer[bytes_read] = '\0';
//             std::cout << "Client " << client->get_fd() << ": " << buffer << std::endl;
//         }
//     }
// }

// void Server::send_message_all(const char *message) {
//     for (size_t i = 0; i < clients.size(); ++i) {
//         Client* client = clients[i];
...
//                 char inputMessage[BUFFER_SIZE];
//                 std::cin.getline(inputMessage, BUFFER_SIZE);
//                 irc.send_message_all(inputMessage);
//             }
